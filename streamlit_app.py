...
/* 조회수 표시 스타일 (상세 페이지) - 추가됨 */
.metric-view {
    font-size: 1.1em;
    font-weight: 700;
    color: #6A5ACD; /* 보라색 계열로 구분 */
}
</style>
"""
...
    post = c.fetchone()
    conn.close()
    # 반환 컬럼: (id, title, content, author, real_author, created_at, likes, views)
    return post 
...
    # --- [추가] 조회수 증가 로직 ---
    # 세션 상태를 사용하여 해당 게시글에 대해 세션 당 한 번만 조회수를 증가시킵니다.
    if 'view_incremented_for_post' not in st.session_state:
        st.session_state.view_incremented_for_post = {}
        
    current_post_views_tracked = st.session_state.view_incremented_for_post.get(post_id, False)

    if not current_post_views_tracked:
        increment_post_views(post_id)
        st.session_state.view_incremented_for_post[post_id] = True
    # -------------------------------

    # [수정] get_post_by_id는 이제 views 컬럼을 포함하여 8개의 데이터를 반환합니다.
...
    st.markdown(f'## {title}')
    
    # [수정] 작성자 및 작성일 정보와 함께 하트/조회수 표시
    st.markdown(f"""
        <div style="font-size: 0.9em; color: #666; margin-bottom: 5px;">
            <span>👤 **작성자:** {author}</span> | 
            <span>🗓️ **작성일:** {created_at}</span>
        </div>
        <div style="display: flex; gap: 20px; align-items: center; margin-bottom: 20px; margin-top: 5px;">
            <span class="metric-heart">❤️ 하트: {likes}</span>
            <span class="metric-view">👀 조회수: {views}</span>
        </div>
...
    c = conn.cursor()
    
    # [수정] school 컬럼을 추가로 선택
    c.execute("SELECT username, password, email, student_id, school, created_at FROM users WHERE username = ?", (st.session_state.username,))
    user = c.fetchone()
    conn.close()

    if user:
        # DB에서 가져온 6개 컬럼 중 password(_)를 제외하고 5개만 사용
        username, _, email, student_id, school, created = user
        st.metric(label="아이디", value=username)
        # [추가] 학교 정보 표시
        st.metric(label="학교", value=school) 
...
        st.session_state.view_incremented_for_post = {} # [추가] 조회수 중복 증가 방지용 상태
...
                st.session_state.page = "login"
                st.session_state.selected_post_id = None
                st.session_state.view_incremented_for_post = {} # 로그아웃 시 초기화
                st.rerun()
        else:
