...
# ✅ 학교 옵션 정의 (추가된 부분)
SCHOOL_OPTIONS = ["대원고등학교", "대원여자고등학교", "대원외국어고등학교"]
...
            # [추가] 학교 선택 항목
            school = st.selectbox("학교 선택", options=SCHOOL_OPTIONS) 
...
    if user:
        # DB에서 가져온 6개 컬럼 중 password(_)를 제외하고 5개 사용
        username, _, email, student_id, school, created = user
        st.metric(label="아이디", value=username)
        st.metric(label="학교", value=school) # [추가] 학교 정보 표시
